...
public abstract class AbstractBuildModelFromConfigAndSecretsTests extends AbstractInferenceServiceParameterizedTests {
        ...
        return Arrays.asList(
            new TestCase[][] {
                ...
                { new TestCaseBuilder("Test failing building model for unsupported task type ", testConfiguration -> {
                    ...
                    return new Utils.ModelConfigAndSecrets(configurationsMock, testConfiguration.commonConfig().createModelSecrets());
                },
    ...
    private static Function<TestConfiguration, Utils.ModelConfigAndSecrets> getTestConfigurationModelConfigAndSecretsFunction(
        ...
        return testConfiguration -> {
            ...
            return new Utils.ModelConfigAndSecrets(modelConfig, commonConfig.createModelSecrets());
        };
    }
    ...
    private void assertSuccessfulModelCreation(SenderService<?> service, Utils.ModelConfigAndSecrets persistedConfig) {
        ...
        assertThat(model.getTaskSettings(), is(testConfiguration.commonConfig().getEmptyTaskSettings(testCase.expectedTaskType)));
        testConfiguration.commonConfig().assertModel(model, testCase.expectedTaskType, true, true, ConfigurationParseContext.PERSISTENT);
    }
