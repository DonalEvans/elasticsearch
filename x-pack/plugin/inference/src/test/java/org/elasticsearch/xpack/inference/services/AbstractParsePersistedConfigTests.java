...
public abstract class AbstractParsePersistedConfigTests extends AbstractInferenceServiceParameterizedTests {
    ...
    private record ServiceParserParams(SenderService<?> service, Utils.PersistedConfig persistedConfig, TestConfiguration testConfig) {}
            ...
            new TestCase[][] {
                    ...
                    new TestCaseBuilder(
                        ...
                        (testConfig, minimalSettings) -> getPersistedConfigMap(
                            testConfig.commonConfig()
                                .createMinimalServiceSettingsMap(
                    ...
                    ).build() },
                {
                    new TestCaseBuilder(
                        ...
                        (testConfig, minimalSettings) -> {
                            var serviceSettings = testConfig.commonConfig()
                                .createMinimalServiceSettingsMap(
                                    testConfig.commonConfig().defaultTaskType(),
                                    ConfigurationParseContext.PERSISTENT
                                );
                    ...
                    ).build() },
                {
                    new TestCaseBuilder(
                        ...
                        (testConfig, minimalSettings) -> {
                            var secretSettingsMap = testConfig.commonConfig().createSecretSettingsMap();
    ...
    private static BiFunction<TestConfiguration, Boolean, Utils.PersistedConfig> persistedConfig(
        ...
        return (testConfig, minimalSettings) -> {
            ...
            return getPersistedConfigMap(
                ...
                withSecrets ? testConfig.commonConfig().createSecretSettingsMap() : null
            );
        };
    }
    ...
    private static ServiceParser getDefaultServiceParser() {
        return params -> params.service.parsePersistedConfig(
            new UnparsedModel(
                INFERENCE_ENTITY_ID,
                params.testConfig.commonConfig().defaultTaskType(),
                "test_service",
                params.persistedConfig.config(),
                params.persistedConfig.secrets()
            )
    ...
    public void testPersistedConfig() throws Exception {
        ...
        var persistedConfig = testCase.createPersistedConfig.apply(testConfiguration, testCase.minimalSettings);
    ...
    private void assertSuccessfulParse(SenderService<?> service, Utils.PersistedConfig persistedConfig) {
        ...
        testConfiguration.commonConfig()
            .assertModel(model, taskType, testCase.modelIncludesSecrets, testCase.minimalSettings, ConfigurationParseContext.PERSISTENT);
    }
